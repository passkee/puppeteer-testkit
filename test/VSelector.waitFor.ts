...
export default () => {
	props0.filter((item) => [ 'val' ].indexOf(item) === -1).forEach((method) => {
		it(`$selector.waitFor.${method}(NOT_EMPTY)`, async () => {
			await $('#for-ppt-test').waitFor[method]($.constants.NOT_EMPTY);
		});
	});

	Object.keys(props1).forEach((method) => {
		it(`$selector.waitFor.${method}(${props1[method]}, NOT_EMPTY)`, async () => {
			await $('#for-ppt-test').waitFor[method](props1[method], $.constants.NOT_EMPTY);
		});
	});
};
